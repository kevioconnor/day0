...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        no_items_in_inventory = len(self.engine.player.inventory.items)
        height = no_items_in_inventory +2
        if height < 3:
            height = 3
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        ...
        if 0 <= index <= 26:
            try:
                item_selected = player.inventory.items[index]
            except IndexError:
                self.engine.message_log.add_message("Invalid entry.", color.invalid)
                return None
            return self.on_item_selected(item_selected)
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: "tcod.event.KeyDown") -> Optional[Action]:
        ...
        if key in MOVE_KEYS:
            dx, dy = MOVE_KEYS[key]
            action = CollideAction(player, dx, dy)
...
class HistoryViewer(EventHandler):
    """Prints the message history on a larger window"""
